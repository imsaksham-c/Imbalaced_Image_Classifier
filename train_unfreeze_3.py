...
class ClassBalancedFocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        weights = self.weights[targets]  # [batch_size]
    ...
    if stage == 1:
        ...
        for name, param in model.named_parameters():
            if 'resnet' in str(type(model)).lower():
                # Unfreeze all of layer3, keep layer4 frozen
                if 'layer3' in name or 'fc' in name:
                    param.requires_grad = True
                else:
                    param.requires_grad = False
            elif 'efficientnet' in str(type(model)).lower():
                # Unfreeze all of features.6, keep features.7 frozen
...
def main():
    parser = argparse.ArgumentParser(description='Temple Image Classifier Training (CB Focal Loss, deeper unfreezing)')
    ...
    criterion = ClassBalancedFocalLoss(samples_per_cls, num_classes, beta=args.beta, gamma=args.gamma)
    print(f"Using Class-Balanced Focal Loss (beta={args.beta}, gamma={args.gamma})")
